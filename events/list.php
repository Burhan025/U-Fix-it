...
if ( !defined('ABSPATH') ) { die('-1'); }
	...
	$eventcat = single_cat_title("", false);
	
	if ($eventcat) { ?>
	
	<!--<h3>Events Category: <?php /* echo $eventcat */ ?></h3> -->
...
<div id="tribe-events-content" class="upcoming">
	...
	<?php if(!tribe_is_day()): // day view doesn't have a grid ?>
		<div id='tribe-events-calendar-header' class="clearfix">
            <select style="float:left" id="calendar-select" name="event-dropdown" onchange='document.location.href=this.options[this.selectedIndex].value;'> 
	<option value=""><?php echo attribute_escape(__('Filter Events by Category')); ?></option> 
	 <?php 
	  $categories=  get_categories('taxonomy=tribe_events_cat'); 
	  foreach ($categories as $category) {
	  	$option = '<option value="' . get_bloginfo('url'). '/events/category/'.$category->category_nicename.'">';
		...
		<div id="post-<?php the_ID() ?>" <?php post_class('tribe-events-event clearfix') ?> itemscope itemtype="http://schema.org/Event">
            ...
            <?php }?>
				...
				<table cellspacing="0">
                    ...
                    <?php if ( get_post_meta(get_the_ID(), '_ecp_custom_1', true) ) : ?>
                    <tr>
						<td class="tribe-events-event-meta-desc"><?php _e('2nd Show:', 'tribe-events-calendar') ?></td>
						<td class="tribe-events-event-meta-value" itemprop="startDate"><?php echo get_post_meta(get_the_ID(), '_ecp_custom_1', true);?></td>
					</tr>
					<?php endif;?>
